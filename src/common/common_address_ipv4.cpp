...
{
	memset(&m_sock_addr, 0, sizeof(struct sockaddr_in));
...
IPAddrV4::IPAddrV4(const std::string &ip, const uint16_t& port)
	:m_ipaddr(ip), m_port(port)
{
	memset(&m_sock_addr, 0, sizeof(struct sockaddr_in));
...
bool IPAddrV4::equals(const std::string& ip, const uint16_t& port)
{
	if(m_ipaddr == ip && m_port == port)
	{
		return true;
	}
